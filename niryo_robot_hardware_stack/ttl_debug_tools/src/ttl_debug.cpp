...
int handleUserInput(int argc, char **argv)
{
  try
  {
    ...
    description.add_options()("help,h", "Print help message")("baudrate,b", po::value<int>()->default_value(1000000),
                                                              "Baud rate")(
        "port,p", po::value<std::string>()->default_value(DEFAULT_PORT),
        "Set port")("id,i", po::value<int>()->default_value(-1),
                    "Motor ID")("ids", po::value<std::vector<int>>()->multitoken(),
                                "list of id for sync read or write")("scan", "Scan all motors on the TTL bus")(
        "ping", "ping specific ID")("get-register", po::value<int>(), "Get a value from a register (arg: reg_addr)")(
        "size", po::value<int>()->default_value(1), "Size (for get-register only)")("calibrate", "calibrate joints")(
        "test", "a test movement")("set-register", po::value<std::vector<int>>()->multitoken(),
                                   "Set a value to a register (args: reg_addr, value, size)")(
        "set-registers", po::value<std::vector<int>>()->multitoken(),
        "Set the values to a register for multiples devices (args: reg_addr, size, values)")(
        "get-registers", po::value<int>()->multitoken(),
        "get the values of a register for multiples devices (arg: reg_addr)");
