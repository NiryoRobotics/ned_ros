...
#define LATENCY_TIMER                                                                                                  \
  16  // msec (USB latency timer)
      // You should adjust the latency timer value. In Windows, the default latency timer of the usb serial is '16
      // msec'. When you are going to use sync / bulk read, the latency timer should be loosen. the lower latency timer
      // value, the faster communication speed.
...
bool PortHandlerWindows::setupPort(int baudrate)
{
  ...
  serial_handle_ =
      CreateFileA(port_name_, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (serial_handle_ == INVALID_HANDLE_VALUE)
  {
  ...
  if (SetupComm(serial_handle_, 4096, 4096) == FALSE)  // Buffer size (Rx,Tx)
    goto DXL_HAL_OPEN_ERROR;
  if (PurgeComm(serial_handle_, PURGE_TXABORT | PURGE_TXCLEAR | PURGE_RXABORT | PURGE_RXCLEAR) == FALSE)  // Clear
                                                                                                          // buffer
    goto DXL_HAL_OPEN_ERROR;
