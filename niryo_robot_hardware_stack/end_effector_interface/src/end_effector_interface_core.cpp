...
void EndEffectorInterfaceCore::initParameters(ros::NodeHandle &nh)
{
  ...
  ROS_DEBUG("EndEffectorInterfaceCore::initParameters - end effector status frequency : %f",
  ...
  while (nh.hasParam("button_" + std::to_string(button_id) + "/type"))
  {
    ...
    ROS_INFO("EndEffectorInterfaceCore::initParameters : configure button %d of type %s", button_id,
             eType.toString().c_str());
...
{
  _free_drive_button_state_publisher =
      nh.advertise<end_effector_interface::EEButtonStatus>("free_drive_button_status", 10, true);

  _save_pos_button_state_publisher =
      nh.advertise<end_effector_interface::EEButtonStatus>("save_pos_button_status", 10, true);

  _custom_button_state_publisher =
      nh.advertise<end_effector_interface::EEButtonStatus>("custom_button_status", 10, true);

  _digital_out_publisher = nh.advertise<end_effector_interface::EEIOState>("io_state", 10, true);

  _states_publisher_timer =
      nh.createTimer(_states_publisher_duration, &EndEffectorInterfaceCore::_publishButtonState, this);
}
...
void EndEffectorInterfaceCore::initEndEffectorHardware()
{
  ...
  if (_end_effector_state && _end_effector_state->isValid())
  {
    ...
    else
    {
      ...
      ROS_WARN(
          "EndEffectorInterfaceCore::initEndEffectorHardware - "
...
int EndEffectorInterfaceCore::initHardware()
{
  if (_end_effector_state)
  {
    ...
    if (_end_effector_state->getHardwareType() == EHardwareType::NED3PRO_END_EFFECTOR)
    {
      ...
      _ttl_interface->addSingleCommandToQueue(std::make_unique<EndEffectorSingleCmd>(
          EEndEffectorCommandType::CMD_TYPE_SET_COLLISION_THRESH_ALGO_2, _end_effector_state->getId(),
          std::initializer_list<uint32_t>{ thresh_algo_2 }));
    }
    _ttl_interface->addSingleCommandToQueue(std::make_unique<EndEffectorSingleCmd>(
        EEndEffectorCommandType::CMD_TYPE_SET_COLLISION_THRESH, _end_effector_state->getId(),
                                                   ...
                                                   end_effector_interface::SetEEDigitalOut::Response &res)
{
  ...
  {
    _ttl_interface->addSingleCommandToQueue(std::make_unique<EndEffectorSingleCmd>(
        EEndEffectorCommandType::CMD_TYPE_DIGITAL_OUTPUT, _end_effector_state->getId(),
        std::initializer_list<uint32_t>{ req.data }));
 ...
 * @return
 */
bool EndEffectorInterfaceCore::_callbackSetDIState(niryo_robot_msgs::SetBool::Request &req,
                                                   niryo_robot_msgs::SetBool::Response &res)
{
  ...
  {
    _ttl_interface->addSingleCommandToQueue(std::make_unique<EndEffectorSingleCmd>(
        EEndEffectorCommandType::CMD_TYPE_DIGITAL_INPUT, _end_effector_state->getId(),
        std::initializer_list<uint32_t>{ req.value }));
