...
FakeJointHardwareInterface::FakeJointHardwareInterface(ros::NodeHandle& nh)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        nh.getParam("joints_interface/joint_" + std::to_string(i + 1) + "_id", _joints_id[i]);
        nh.getParam("joints_interface/joint_" + std::to_string(i + 1) + "_name", _joints_name[i]);
    }

    ROS_INFO("Fake Joint Interface - Joints' Name : (1 : %s, 2 : %s, 3 : %s)",
             _joints_name[0].c_str(), _joints_name[1].c_str(), _joints_name[2].c_str());

    ROS_INFO("Fake Joint Interface - Joints' Name : (4 : %s, 5 : %s, 6 : %s)",
             _joints_name[3].c_str(), _joints_name[4].c_str(), _joints_name[5].c_str());

    ROS_INFO("Fake Joint Interface - Joints' ID : (1 : %d, 2 : %d, 3 : %d)",
             _joints_id[0], _joints_id[1], _joints_id[2]);

    ROS_INFO("Fake Joint Interface - Joints' ID : (4 : %d, 5 : %d, 6 : %d)",
             _joints_id[3], _joints_id[4], _joints_id[5]);

    // connect and register joint state interface
    std::vector<hardware_interface::JointStateHandle> state_handle;
    for (size_t i = 0; i < 6; i++)
    {
        state_handle.emplace_back(hardware_interface::JointStateHandle(
                                      _joints_name[i], &_pos[i], &_vel[i], &_eff[i]));
    ...
    for (size_t i = 0; i < 6; i++)
    {
        position_handle.emplace_back(hardware_interface::JointHandle(
                                         _joint_state_interface.getHandle(_joints_name[i]), &_cmd[i]));
