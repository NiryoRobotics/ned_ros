 ...
 * @param ttl_interface
 */
ToolsInterfaceCore::ToolsInterfaceCore(ros::NodeHandle &nh, std::shared_ptr<ttl_driver::TtlInterfaceCore> ttl_interface)
  ...
  if (_toolState && _toolState->isValid())
  {
    ...
    if (res)
      initHardware(torque_on, _temperature_limit, _shutdown_configuration,
                   _available_tools_map.at(_toolState->getId()));
  }
                                     ...
                                     const ToolConfig &config)
{
  ...
  if (_toolState)
  {
    ...
    {
      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(EDxlCommandType::CMD_TYPE_TORQUE, motor_id,
                                                                             std::initializer_list<uint32_t>{ false }));

      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
          EDxlCommandType::CMD_TYPE_TEMPERATURE_LIMIT, motor_id, std::initializer_list<uint32_t>{ temperature_limit }));
      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
          EDxlCommandType::CMD_TYPE_SHUTDOWN, motor_id, std::initializer_list<uint32_t>{ shutdown_configuration }));

      uint8_t control_mode = 5;  // torque + position
      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
          EDxlCommandType::CMD_TYPE_CONTROL_MODE, motor_id, std::initializer_list<uint32_t>{ control_mode }));
      ...
      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
          EDxlCommandType::CMD_TYPE_PID, motor_id,
          std::initializer_list<uint32_t>{ _toolState->getPositionPGain(), _toolState->getPositionIGain(),
                                           _toolState->getPositionDGain(), _toolState->getVelocityPGain(),
                                           _toolState->getVelocityIGain(), _toolState->getFF1Gain(),
      ...
      _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
          EDxlCommandType::CMD_TYPE_PROFILE, motor_id,
          std::initializer_list<uint32_t>{ _toolState->getVelProfile(), _toolState->getAccProfile() }));
    }
    // TORQUE cmd on if ned2, off otherwise
    _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
        EDxlCommandType::CMD_TYPE_TORQUE, motor_id, std::initializer_list<uint32_t>{ torque_on }));
  }
...
void ToolsInterfaceCore::initParameters(ros::NodeHandle &nh)
{
  ...
  if (idList.size() == typeList.size() && idList.size() == nameList.size())
  {
    ...
    for (size_t i = 0; i < idList.size(); ++i)
    {
      ...
      if (!_available_tools_map.count(id))
      {
        ...
        {
          auto tool_config = ToolConfig{ name, type, ToolParams{ velocity_profile, acceleration_profile, pid } };

          _available_tools_map.insert(std::make_pair(id, tool_config));
        }
        else
          ROS_ERROR(
              "ToolsInterfaceCore::initParameters - unknown type %s. "
              "Please check your configuration file (tools_interface/config/default.yaml)",
              typeList.at(id).c_str());
      }
      else
        ROS_ERROR(
            "ToolsInterfaceCore::initParameters - duplicate id %d. "
            "Please check your configuration file (tools_interface/config/default.yaml)",
            id);
    }
  }
  ...
  {
    ROS_ERROR(
        "ToolsInterfaceCore::initParameters - wrong dynamixel configuration. "
        "Please check your configuration file (tools_interface/config/default.yaml)");
  }

  for (auto const &tool : _available_tools_map)
  {
    ROS_DEBUG("ToolsInterfaceCore::initParameters - Available tools map: %d => %s (%s)", static_cast<int>(tool.first),
              tool.second.name.c_str(), HardwareTypeEnum(tool.second.type).toString().c_str());
  }
}
...
void ToolsInterfaceCore::startServices(ros::NodeHandle &nh)
{
  ...
  _open_gripper_server =
      nh.advertiseService("/niryo_robot/tools/open_gripper", &ToolsInterfaceCore::_callbackControlGripper, this);

  _close_gripper_server =
      nh.advertiseService("/niryo_robot/tools/close_gripper", &ToolsInterfaceCore::_callbackControlGripper, this);

  _control_gripper_server =
      nh.advertiseService("/niryo_robot/tools/control_gripper", &ToolsInterfaceCore::_callbackControlGripper, this);
...
void ToolsInterfaceCore::startPublishers(ros::NodeHandle &nh)
{
  ...
  _tool_connection_publisher_timer =
      nh.createTimer(_tool_connection_publisher_duration, &ToolsInterfaceCore::_publishToolConnection, this);
}
                                                 ...
                                                 tools_interface::PingDxlTool::Response &res)
{
  ...
  for (auto const &m_id : motor_list)
  {
    if (_available_tools_map.count(m_id))
    {
      // reset tool state
      _toolState =
          std::make_shared<ToolState>(_available_tools_map.at(m_id).name, _available_tools_map.at(m_id).type, m_id);
      break;
    }
  }
  ...
  if (_toolState)
  {
    if (_toolState->isValid())
    {
      ...
      for (int tries = 0; tries < 3; tries++)
      {
        ...
        ROS_WARN(
            "ToolsInterfaceCore::_callbackPingAndSetDxlTool - "
 ...
 * @return
 */
bool ToolsInterfaceCore::_callbackToolReboot(niryo_robot_msgs::Trigger::Request & /*req*/,
 ...
 * @return
 */
bool ToolsInterfaceCore::_callbackControlGripper(tools_interface::ToolCommand::Request &req,
...
void ToolsInterfaceCore::_waitForToolStop(int id, int timeout)
{
  ...
  while (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time).count() <
         timeout)
  {
    ...
    // If the tool has been detected as not moving 10 times in a row, we consider that the vacuum pump cannot move
    // anymore
    if (moving_counter <= 0)
    {
      break;
...
void ToolsInterfaceCore::_toolCommand(uint32_t position, int torque, uint32_t velocity)
{
  ...
  if (EHardwareType::XL320 == tool_motor_type)
    _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
        EDxlCommandType::CMD_TYPE_VELOCITY, tool_id, std::initializer_list<uint32_t>{ velocity }));

  // need to set max torque. It makes cmd changing speed take effect
  _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(
      EDxlCommandType::CMD_TYPE_EFFORT, tool_id, std::initializer_list<uint32_t>{ static_cast<uint16_t>(torque) }));

  _ttl_interface->addSingleCommandToQueue(std::make_unique<DxlSingleCmd>(EDxlCommandType::CMD_TYPE_POSITION, tool_id,
                                                                         std::initializer_list<uint32_t>{ position }));
}
