...
struct VelocityProfile
{
  ...
  {
    return { v_start, a_1, v_1, a_max, v_max, d_max, d_1, v_stop };
  }
};
...
public:
  ...
  StepperMotorState(EHardwareType type, EComponentType component_type, EBusProtocol bus_proto, uint8_t id);
  StepperMotorState(std::string name, EHardwareType type, EComponentType component_type, EBusProtocol bus_proto,
                    uint8_t id);
