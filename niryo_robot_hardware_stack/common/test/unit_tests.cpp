...
{
protected:
  DXLCommonTest() : dxlState{ std::get<0>(GetParam()), std::get<1>(GetParam()), 1 }
  {
...
{
protected:
  DXLIdentityRadTest() : dxlState{ std::get<0>(GetParam()), std::get<1>(GetParam()), 1 }
  {
    std::cout << "Test for MotorType " << HardwareTypeEnum(std::get<0>(GetParam())).toString() << std::endl;
...
{
protected:
  DXLIdentityMotorTest() : dxlState{ std::get<0>(GetParam()), std::get<1>(GetParam()), 1 }
  {
    std::cout << "Test for MotorType " << HardwareTypeEnum(std::get<0>(GetParam())).toString() << std::endl;
...
TEST_P(DXLCommonTest, extremeHigh)
{
  ...
  EXPECT_NEAR(dxlState.to_rad_pos(dxlState.getTotalRangePosition()), totalAngle_rad + dxlState.getOffsetPosition(),
              precision)
      << "to_motor_pos failed";
}
...
TEST_P(DXLIdentityRadTest, identityFromRad)
{
  ...
  if (test_rad > dxlState.getLimitPositionMax())
    ...
    EXPECT_NEAR(dxlState.to_rad_pos(dxlState.to_motor_pos(test_rad)), test_rad, precision) << "to_rad_pos o "
                                                                                              "to_motor_pos is not "
                                                                                              "identity";
}
...
TEST_P(DXLIdentityMotorTest, identityFromMotorPos)
{
      ...
      dxlState.to_rad_pos(test_pos) < dxlState.getLimitPositionMin())
  {
    ...
    EXPECT_EQ(dxlState.to_motor_pos(dxlState.to_rad_pos(test_pos)), test_pos) << "to_motor_pos o to_rad_pos is not "
                                                                                 "identity";
}

INSTANTIATE_TEST_CASE_P(CommonTests, DXLCommonTest,
                        testing::Combine(testing::Values(EHardwareType::XL320, EHardwareType::XL330,
                                                         EHardwareType::XL430, EHardwareType::XC430),
                                         testing::Values(EComponentType::JOINT, EComponentType::END_EFFECTOR,
                                                         EComponentType::TOOL),
...
INSTANTIATE_TEST_CASE_P(IdentityRadTests, DXLIdentityRadTest,
                        testing::Combine(testing::Values(EHardwareType::XL320, EHardwareType::XL330,
                                         ...
                                         testing::Values(EComponentType::JOINT, EComponentType::END_EFFECTOR,
                                                         EComponentType::TOOL),
                                         testing::Range(-2.0, 2.0, 0.6), testing::Range(-M_PI, M_PI, 0.29)));
...
INSTANTIATE_TEST_CASE_P(IdentityMotorTests, DXLIdentityMotorTest,
                        testing::Combine(testing::Values(EHardwareType::XL320, EHardwareType::XL330,
                                         ...
                                         testing::Values(EComponentType::JOINT, EComponentType::END_EFFECTOR,
                                                         EComponentType::TOOL),
                                         testing::Range(-2.0, 2.0, 0.6), testing::Range(0, 4000, 600)));
...
INSTANTIATE_TEST_CASE_P(IdentityRadTest, StepperIdentityRadTest,
                        testing::Combine(testing::Range(-M_PI, M_PI, M_PI / 4), testing::Range(5.0, 500.0, 50.0),
                                         testing::Values(-1, 1)));
/**
 * @param pos
 * @param gear_ratio
...
TEST_P(StepperIdentityMotorTest, identityFromRad)
{
  ...
  if (stepperState.to_rad_pos(pos) > stepperState.getLimitPositionMax() ||
    ...
    EXPECT_NEAR(stepperState.to_motor_pos(stepperState.to_rad_pos(pos)), pos, precision) << "to_motor_pos o to_rad_pos "
                                                                                            "is not identity";
}

INSTANTIATE_TEST_CASE_P(IdentityMotorTest, StepperIdentityMotorTest,
                        testing::Combine(testing::Range(0, 2000, 500), testing::Range(5.0, 500.0, 50.0),
                                         testing::Values(-1, 1)));
...
TEST(CommonTestSuite, testCreationDxlCmd)
{
  ...
  common::model::DxlSingleCmd dxlCmd(common::model::EDxlCommandType::CMD_TYPE_POSITION, 5,
                                     std::initializer_list<uint32_t>{ 5 });
...
{
  // DxlSingleCmd valid param
  common::model::StepperTtlSingleCmd cmd(common::model::EStepperCommandType::CMD_TYPE_POSITION, 1,
                                         std::initializer_list<uint32_t>{ 5 });
...
{
  // DxlSingleCmd valid param
  common::model::StepperSingleCmd cmd(common::model::EStepperCommandType::CMD_TYPE_POSITION, 1,
                                      std::initializer_list<int32_t>{ 5 });
  ASSERT_TRUE(cmd.isValid());
  ASSERT_TRUE(cmd.isStepperCmd());
