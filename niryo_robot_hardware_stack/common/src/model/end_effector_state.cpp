...
EndEffectorState::EndEffectorState(uint8_t id, common::model::EHardwareType type)
  : AbstractHardwareState(type, EComponentType::END_EFFECTOR, EBusProtocol::TTL, id)
{
...
{
  return (EHardwareType::UNKNOWN != getHardwareType() && EButtonType::UNKNOWN != _buttons_list.at(0)->type &&
          EButtonType::UNKNOWN != _buttons_list.at(1)->type && EButtonType::UNKNOWN != _buttons_list.at(2)->type);
}
...
void EndEffectorState::setButtonStatus(uint8_t button_id, EActionType action)
{
  ...
  else if (action == EActionType::LONG_PUSH_ACTION ||
           (action == EActionType::HANDLE_HELD_ACTION && !button->needsToSkip()))
  {
    button->actions.push(action);
  }
}
 ...
 * @param thresh
 */
void EndEffectorState::setCollisionThreshAlgo2(int thresh)
{
  _collision_thresh_algo_2 = thresh;
}
