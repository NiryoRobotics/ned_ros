...
TEST_F(CanManagerTestSuite, addAndRemoveMotor)
{
  ...
  common::model::StepperMotorState new_state(common::model::EHardwareType::STEPPER,
                                             common::model::EComponentType::JOINT, common::model::EBusProtocol::CAN,
                                             10);
...
{
  auto cmd_1 = std::make_unique<common::model::StepperSingleCmd>(common::model::EStepperCommandType::CMD_TYPE_TORQUE, 2,
                                                                 std::initializer_list<int32_t>{ 1 });
  EXPECT_EQ(can_manager->writeSingleCommand(std::move(cmd_1)), CAN_OK);
  ros::Duration(0.01).sleep();

  auto cmd_2 = std::make_unique<common::model::StepperSingleCmd>(common::model::EStepperCommandType::CMD_TYPE_TORQUE, 2,
  ...
  // wrong id
  auto cmd_3 = std::make_unique<common::model::StepperSingleCmd>(common::model::EStepperCommandType::CMD_TYPE_TORQUE, 7,
                                                                 std::initializer_list<int32_t>{ 1 });
  EXPECT_NE(can_manager->writeSingleCommand(std::move(cmd_3)), CAN_OK);
  ros::Duration(0.01).sleep();

  // wrong type cmd
  auto cmd_4 = std::make_unique<common::model::StepperSingleCmd>(common::model::EStepperCommandType::CMD_TYPE_UNKNOWN,
                                                                 2, std::initializer_list<int32_t>{ 1 });
