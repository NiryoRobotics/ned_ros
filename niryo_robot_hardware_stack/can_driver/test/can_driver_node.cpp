...
class CanDriverTest
{
    ...
    public:
        CanDriverTest()
        {
            ...
            _can_driver.reset(new can_driver::CanDriverCore());

            ros::Duration(2).sleep();
            for (auto const& sState : states)
            {
                common::model::StepperMotorCmd cmd_torque(common::model::EStepperCommandType::CMD_TYPE_TORQUE,
                                                          sState.getId(), {false});
                _can_driver->addSingleCommandToQueue(cmd_torque);
            }
        ...
        void TestServiceActiveTorque()
        {
            ...
            for (auto const& sState : states)
            {
                common::model::StepperMotorCmd cmd(common::model::EStepperCommandType::CMD_TYPE_TORQUE,
                                                   sState.getId(), {true});
        ...
        void TestPublishPoseCmd()
        {
            ...
            {
                common::model::StepperMotorCmd cmd(common::model::EStepperCommandType::CMD_TYPE_POSITION,
                                                   states.at(j).getId(), {home_pose.at(j)});
                _can_driver->addSingleCommandToQueue(cmd);
            }
            ros::Duration(5).sleep();
        ...
        void GetState()
        {
            ...
            std::vector<std::shared_ptr<common::model::StepperMotorState> > stepper_motor_state;
            while (ros::ok())
            {
        ...
        void TestReceiveState()
        {
            ...
            double p = 1.5;
            std::vector<std::shared_ptr<common::model::StepperMotorState> > stepper_motor_states;
            ...
            while (std::abs(error) > 20)
            {
                ...
                cmd_send.addMotorParam(stepper_motor_states.at(0), cmd.at(0) + error);

                cmd_send.addMotorParam(stepper_motor_states.at(1),
                                       static_cast<int32_t>(stepper_motor_states.at(1)->getPositionState()));

                cmd_send.addMotorParam(stepper_motor_states.at(2),
                                       static_cast<int32_t>(stepper_motor_states.at(2)->getPositionState()));
