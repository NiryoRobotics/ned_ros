...
int CanInterfaceCore::motorCmdReport(const JointState &jState, common::model::EHardwareType motor_type)
{
  ...
  if (motor_type != EHardwareType::UNKNOWN)
  {
    if (_debug_flag)
    {
      ...
      _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(
          EStepperCommandType::CMD_TYPE_TORQUE, motor_id, std::initializer_list<int32_t>{ 1 }));
      ros::Duration(0.5).sleep();
      ...
      _can_manager->writeSingleCommand(
          std::make_unique<StepperSingleCmd>(EStepperCommandType::CMD_TYPE_RELATIVE_MOVE, motor_id,
      ...
      _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(
          EStepperCommandType::CMD_TYPE_TORQUE, motor_id, std::initializer_list<int32_t>{ 0 }));
...
int CanInterfaceCore::launchMotorsReport()
{
  ...
  if (_debug_flag)
  {
    ...
    {
      ROS_INFO("%d \t| %s \t| %d", std::get<0>(res), HardwareTypeEnum(std::get<1>(res)).toString().c_str(),
               std::get<2>(res));
    }
  }
  else
...
void CanInterfaceCore::activeDebugMode(bool mode)
{
  ...
  if (!mode)
  {
    ...
    _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(EStepperCommandType::CMD_TYPE_TORQUE, 2,
                                                                        std::initializer_list<int32_t>{ 0 }));
    ros::Duration(0.2).sleep();

    // disable torque on motor 3
    _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(EStepperCommandType::CMD_TYPE_TORQUE, 3,
                                                                        std::initializer_list<int32_t>{ 0 }));
  }
}
...
void CanInterfaceCore::controlLoop()
{
  ...
  while (ros::ok())
  {
    ...
    else if (_control_loop_flag)
    {
      ...
      if (!isFreqMet)
        ROS_DEBUG_THROTTLE(2, "CanInterfaceCore::rosControlLoop : freq not met : expected (%f s) vs actual (%f s)",
                           control_loop_rate.expectedCycleTime().toSec(), control_loop_rate.cycleTime().toSec());
    }
    else
    {
...
int CanInterfaceCore::setConveyor(const std::shared_ptr<common::model::ConveyorState> &state)
{
  ...
  if (res)
  {
    ...
    _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(
        EStepperCommandType::CMD_TYPE_MICRO_STEPS, state->getId(),
        std::initializer_list<int32_t>{ static_cast<int32_t>(state->getMicroSteps()) }));

    _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(
        EStepperCommandType::CMD_TYPE_MAX_EFFORT, state->getId(),
        std::initializer_list<int32_t>{ static_cast<int32_t>(state->getMaxEffort()) }));

    _can_manager->writeSingleCommand(std::make_unique<StepperSingleCmd>(
        EStepperCommandType::CMD_TYPE_CONVEYOR, state->getId(), std::initializer_list<int32_t>{ false, 0, -1 }));
...
void CanInterfaceCore::addSingleCommandToQueue(std::unique_ptr<common::model::ISingleMotorCmd> &&cmd)  // NOLINT
{
  ...
  if (cmd && cmd->isValid())
  {
    if (cmd->getCmdType() == static_cast<int>(EStepperCommandType::CMD_TYPE_CONVEYOR))
    {
      ...
      {
        ROS_WARN("CanInterfaceCore::addCommandToQueue: Cmd queue overflow ! %d",
                 static_cast<int>(_conveyor_cmds.size()));
      }
      else
      {
        _conveyor_cmds.push(
            common::util::static_unique_ptr_cast<common::model::AbstractCanSingleMotorCmd>(std::move(cmd)));
      }
    }
    else
    {
      ...
      {
        ROS_WARN("CanInterfaceCore::addCommandToQueue: Cmd queue overflow ! %d",
                 static_cast<int>(_stepper_single_cmds.size()));
      }
      else
      {
        _stepper_single_cmds.push(
            common::util::static_unique_ptr_cast<common::model::AbstractCanSingleMotorCmd>(std::move(cmd)));
      }
    }
  }
 ...
 */
std::vector<uint8_t> CanInterfaceCore::getRemovedMotorList() const
{
