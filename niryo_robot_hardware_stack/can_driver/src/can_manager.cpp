...
/**
 * @brief CanManager::_verifyMotorTimeoutLoop : check that motors are still visible in the duration defined by
 * getCurrentTimeout() This timeout changed overtime according to the current state of the driver (in calibration or
 * not)
 */
      ...
      for (auto const &map_it : _state_map)
      {
        if (map_it.second && map_it.second->getComponentType() != common::model::EComponentType::CONVEYOR)
        {
          ...
          if (map_it.second && map_it.second->isValid())
          {
            ...
            }  // else, if it is not in the list of connected motors, we add it (? _all_motor_connected got from scan,
               // why add if id of state not found)
...
void CanManager::startCalibration()
{
  ...
  {
    if (s.second &&
        (EHardwareType::STEPPER == s.second->getHardwareType() ||
         EHardwareType::FAKE_STEPPER_MOTOR == s.second->getHardwareType()) &&
...
void CanManager::resetCalibration()
{
  ...
  {
    if (s.second &&
        (s.second->getHardwareType() == EHardwareType::STEPPER ||
         s.second->getHardwareType() == EHardwareType::FAKE_STEPPER_MOTOR) &&
        !std::dynamic_pointer_cast<StepperMotorState>(s.second)->isConveyor())
      std::dynamic_pointer_cast<StepperMotorState>(s.second)->setCalibration(EStepperCalibrationStatus::UNINITIALIZED,
                                                                             0);
  }
}
...
{
  EStepperCalibrationStatus newStatus =
      _calibration_status > EStepperCalibrationStatus::OK ? EStepperCalibrationStatus::OK : _calibration_status;
