  ...
  if (cmd->isValid() && cmd->isStepperCmd())
  {
    switch (EStepperCommandType(cmd->getCmdType()))
    {
      ...
      case EStepperCommandType::CMD_TYPE_CALIBRATION:
        return sendCalibrationCommand(cmd->getId(), cmd->getParams().at(0), cmd->getParams().at(1),
                                      cmd->getParams().at(2), cmd->getParams().at(3));
      ...
      case EStepperCommandType::CMD_TYPE_CONVEYOR:
        return sendConveyorOnCommand(cmd->getId(), cmd->getParams().at(0), static_cast<uint8_t>(cmd->getParams().at(1)),
