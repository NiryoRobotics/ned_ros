...
public:
  ...
  int syncWriteHomingAbsPosition(const std::vector<uint8_t> &id_list,
                                 const std::vector<uint32_t> &abs_position) override;
...
int StepperDriver<reg_type>::checkModelNumber(uint8_t id, uint16_t &model_number)
{
  ...
  if (ping_result == COMM_SUCCESS && model_number != 0)
  {
    ...
    {
      ROS_WARN("StepperDriver: Model number %u not valid for %s (expected %s)", model_number,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncWriteTorquePercentage(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncWritePositionGoal(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncWriteVelocityGoal(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncReadFirmwareVersion(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncReadTemperature(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncReadHwErrorStatus(const std::vector<uint8_t> &id_list,
...
int StepperDriver<reg_type>::readVelocityProfile(uint8_t id, std::vector<uint32_t> &data_list)
{
  ...
  if (COMM_SUCCESS ==
      syncReadConsecutiveBytes<typename reg_type::TYPE_PROFILE, 8>(reg_type::ADDR_VSTART, { id }, data_array_list))
  {
...
int StepperDriver<reg_type>::writeHomingSetup(uint8_t id, uint8_t direction, uint8_t stall_threshold)
{
  ...
  // if (COMM_SUCCESS != write<typename reg_type::TYPE_HOMING_STALL_THRESHOLD>(reg_type::ADDR_HOMING_STALL_THRESHOLD,
  // id, stall_threshold))
  //     res++;
  // ros::Duration(wait_duration).sleep();
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncReadHomingStatus(const std::vector<uint8_t> &id_list,
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncWriteHomingAbsPosition(const std::vector<uint8_t> &id_list,
                                                        const std::vector<uint32_t> &abs_position)
{
  return syncWrite<typename reg_type::TYPE_HOMING_ABS_POSITION>(reg_type::ADDR_HOMING_ABS_POSITION, id_list,
                                                                abs_position);
}
 ...
 */
template <typename reg_type>
int StepperDriver<reg_type>::syncReadHomingAbsPosition(const std::vector<uint8_t> &id_list,
                                                       std::vector<uint32_t> &abs_position)
{
  return syncRead<typename reg_type::TYPE_HOMING_ABS_POSITION>(reg_type::ADDR_HOMING_ABS_POSITION, id_list,
                                                               abs_position);
}
