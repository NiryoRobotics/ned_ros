...
public:
  ...
  virtual int syncReadFirmwareVersion(const std::vector<uint8_t> &id_list,
                                      std::vector<std::string> &firmware_version) = 0;
  ...
  virtual int syncReadHwStatus(const std::vector<uint8_t> &id_list,
                               std::vector<std::pair<double, uint8_t> > &data_array_list) = 0;
...
protected:
  ...
  int syncReadConsecutiveBytes(uint16_t address, const std::vector<uint8_t> &id_list,
...
int AbstractTtlDriver::read(uint16_t address, uint8_t id, T &data)
{
  ...
  switch (sizeof(T))
  {
    case DXL_LEN_ONE_BYTE: {
      ...
      dxl_comm_result = _dxlPacketHandler->read1ByteTxRx(_dxlPortHandler.get(), id, address, &raw_data, &error);
    ...
    case DXL_LEN_TWO_BYTES: {
      ...
      dxl_comm_result = _dxlPacketHandler->read2ByteTxRx(_dxlPortHandler.get(), id, address, &raw_data, &error);
      data = static_cast<T>(raw_data);
    }
    break;
    case DXL_LEN_FOUR_BYTES: {
      ...
      dxl_comm_result = _dxlPacketHandler->read4ByteTxRx(_dxlPortHandler.get(), id, address, &raw_data, &error);
      data = static_cast<T>(raw_data);
    }
    break;
    default:
  ...
  if (0 != error)
  {
    printf("AbstractTtlDriver::write ERROR: device return error: id=%d, addr=%d, len=%d, err=0x%02x\n", id, address,
           static_cast<int>(sizeof(T)), error);
 ...
 */
template <typename T, const size_t N>
int AbstractTtlDriver::syncReadConsecutiveBytes(uint16_t address, const std::vector<uint8_t> &id_list,
                                                std::vector<std::array<T, N> > &data_list)
{
  data_list.clear();
 ...
 */
template <typename T>
int AbstractTtlDriver::syncRead(uint16_t address, const std::vector<uint8_t> &id_list, std::vector<T> &data_list)
{
  int dxl_comm_result = COMM_TX_FAIL;
...
int AbstractTtlDriver::write(uint16_t address, uint8_t id, T data)
{
  ...
  {
    case DXL_LEN_ONE_BYTE:
      dxl_comm_result = _dxlPacketHandler->write1ByteTxRx(_dxlPortHandler.get(), id, address, data, &error);
      break;
    case DXL_LEN_TWO_BYTES:
      dxl_comm_result = _dxlPacketHandler->write2ByteTxRx(_dxlPortHandler.get(), id, address, data, &error);
      break;
    case DXL_LEN_FOUR_BYTES:
      dxl_comm_result = _dxlPacketHandler->write4ByteTxRx(_dxlPortHandler.get(), id, address, data, &error);
      break;
    default:
    ...
    if (id_list.size() == data_list.size())
    {
      ...
      for (size_t i = 0; i < id_list.size(); ++i)
      {
        ...
        switch (data_len)
        {
          ...
          break;
          case DXL_LEN_FOUR_BYTES: {
            uint8_t params[4] = { DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)),
