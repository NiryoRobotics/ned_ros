...
ConveyorInterfaceCore::ConveyorInterfaceCore(ros::NodeHandle &nh,
                                             shared_ptr<ttl_driver::TtlInterfaceCore> ttl_interface,
                                             shared_ptr<can_driver::CanInterfaceCore> can_interface)
...
void ConveyorInterfaceCore::startServices(ros::NodeHandle &nh)
{
  ...
  _get_hardware_id_server = nh.advertiseService("/niryo_robot/conveyor/get_hardware_id",
                                                &ConveyorInterfaceCore::_callbackGetHardwareId, this);
}
...
{
  _conveyors_feedback_publisher =
      nh.advertise<conveyor_interface::ConveyorFeedbackArray>("/niryo_robot/conveyor/feedback", 10, true);

  _publish_conveyors_feedback_timer =
      nh.createTimer(ros::Duration(_publish_feedback_duration),
                     [this](const ros::TimerEvent &) { this->_publishConveyorsFeedback(); });
}
...
conveyor_interface::SetConveyor::Response ConveyorInterfaceCore::addConveyor()
{
  ...
  for (auto &[protocol, bus] : _bus_config_map)
  {
    ...
    if (bus.isValid())
    {
      ...
      auto conveyor_state =
          std::make_shared<ConveyorState>(hw_type, protocol, bus.default_id, bus.default_id, CONVEYOR_V3_HWID);
      ...
      for (int tries = 0; tries < 3; tries++)
      {
        ...
        if (result == niryo_robot_msgs::CommandStatus::HARDWARE_NOT_SUPPORTED)
        {
          ...
          conveyor_state = std::make_shared<ConveyorState>(EHardwareType::STEPPER, protocol, bus.default_id,
        ...
        if (niryo_robot_msgs::CommandStatus::SUCCESS == result &&
            niryo_robot_msgs::CommandStatus::SUCCESS == initHardware(conveyor_state))
        {
...
int ConveyorInterfaceCore::initHardware(shared_ptr<ConveyorState> conveyor_state)
{
  ...
  if (hwType == EHardwareType::NED3PRO_STEPPER)
  {
    ...
    interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
        EStepperCommandType::CMD_TYPE_VELOCITY_PROFILE, conveyor_state->getId(), velocity_profile.to_list()));
    // TORQUE cmd
    constexpr auto CONVEYOR_TORQUE_PERCENT = 40;
    interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
        EStepperCommandType::CMD_TYPE_TORQUE, conveyor_state->getId(),
...
conveyor_interface::SetConveyor::Response ConveyorInterfaceCore::removeConveyor(uint8_t id)
{
  ...
  auto it = std::find_if(_conveyor_state_list.begin(), _conveyor_state_list.end(),
                         [id](std::shared_ptr<ConveyorState> c) { return (c->getId() == id); });
  ...
  if (it != _conveyor_state_list.end() && *it)
  {
    ...
    if (EBusProtocol::CAN == bus_proto)
    {
      _can_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(
          ...
          std::initializer_list<int32_t>{ req.control_on, req.speed, req.direction * assembly_direction }));
    }
    else if (EBusProtocol::TTL == bus_proto)
    {
      ...
      _ttl_interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
          EStepperCommandType::CMD_TYPE_CONVEYOR, req.id,
          std::initializer_list<uint32_t>{ req.control_on, static_cast<uint32_t>(speed),
...
bool ConveyorInterfaceCore::_callbackGetHardwareId(conveyor_interface::GetHardwareId::Request &req,
      ...
      if (interface)
      {
        ...
        else
        {
          ...
          ROS_DEBUG_THROTTLE(2.0,
                             "ConveyorInterfaceCore::_publishConveyorsFeedback - Found a conveyor, publishing data : "
                             "%s",
                             conveyor_state->str().c_str());
        }
      }
    }
  }
