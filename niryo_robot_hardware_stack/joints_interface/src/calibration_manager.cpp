...
namespace joints_interface
{
    ...
    EStepperCalibrationStatus CalibrationManager::autoCalibration()
    {
        ...
        if ("ned2" == _hardware_version && readHomingAbsPositionFromFile())
        {
            ...
            ros::Duration(10).sleep();  // TODO: erase line when finished testing
        }
        ...
        if(_ttl_interface)
        {
            ...
            for (auto const &jState : _joint_states_list)
            {
                if (jState && jState->isStepper())
                {
                    ...
                    ROS_DEBUG("CalibrationManager::writeHomingAbsPosition - id: %d, homing: %ud", state->getId(), static_cast<int32_t>(state->getHomingAbsPosition()));
                }
            }
    ...
    void CalibrationManager::readHomingAbsPosition(std::vector<int> &homing_abs_position_ids, std::vector<int> &homing_abs_position_results)
    {
        ...
        _ttl_interface->readHomingAbsPosition();
    ...
    bool CalibrationManager::saveHomingAbsPositionToFile(const std::vector<int> &homing_abs_position_ids, const std::vector<int32_t> &homing_abs_position_results)
    {
        ...
        if (!returned_error)
        {
            ...
            {
                text_to_write += std::to_string(homing_abs_position_ids.at(i));
                text_to_write += ":";
                text_to_write += std::to_string(homing_abs_position_results.at(i));
                if (i < homing_abs_position_ids.size() - 1)
                {
                    text_to_write += "\n";
                }
            }
    ...
    bool CalibrationManager::readHomingAbsPositionFromFile()
    {
        ...
        if (offset_file.is_open())
        {
            ...
            while (getline(offset_file, current_line))
            {
                ...
                {
                    size_t index = current_line.find(':');
                    ROS_DEBUG("read SD card values: %s", current_line.c_str());
