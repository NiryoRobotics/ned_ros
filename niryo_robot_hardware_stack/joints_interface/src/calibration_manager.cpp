...
void CalibrationManager::initParameters(ros::NodeHandle &nh)
{
  ...
  for (int currentIdStepper = 1; nh.hasParam("calibration_params/stepper_" + std::to_string(currentIdStepper) + "/id");
       ++currentIdStepper)
  {
    // first get id
    ...
    ROS_DEBUG("Calibration Interface::initParameters - Calibration Profile: {%d, %d, %d, %d, %d, %d, %d, %d}",
              profile.v_start, profile.a_1, profile.v_1, profile.a_max, profile.v_max, profile.d_max, profile.d_1,
              profile.v_stop);
...
int CalibrationManager::startCalibration(int mode, std::string &result_message)
{
  ...
  ROS_ERROR_COND(niryo_robot_msgs::CommandStatus::SUCCESS != res, "Calibration Interface - Calibration error : %s",
                 result_message.c_str());

  return res;
}

int CalibrationManager::startFactoryCalibration(FactoryCalibration::Request::_command_type command,
                                                FactoryCalibration::Request::_ids_type ids, std::string &result_message)
{
      ...
      (_stepper_bus_interface && _stepper_bus_interface->isConnectionOk()))
  {
    ...
    for (const auto &id : ids)
    {
      ...
      if (std::find_if(_joint_states_list.begin(), _joint_states_list.end(),
                       [id](const std::shared_ptr<common::model::JointState> joint_state) {
                         return joint_state->getId() == id && joint_state->isStepper();
                       }) != _joint_states_list.end())
      {
      ...
      if (mState)
      {
        ...
        if (!firmware_version.empty())
        {
          if (stoi(firmware_version.substr(0, 1)) >= 2)
          {
            ...
            if (readCalibrationOffsetsFromFile(motor_id_list, steps_list))
            {
              ...
              if (!found)
              {
                result_message = "Calibration Interface - Motor " + std::to_string(mState->getId()) +
                                 " does not have a saved offset value, " + "you need to do one auto calibration";
            ...
            {
              result_message =
                  "Calibration Interface - You need to make an "
        ...
        else
        {
          result_message = "Calibration Interface - No firmware version available for motor " +
                           std::to_string(mState->getId()) + ". Make sure all motors are connected";
...
EStepperCalibrationStatus CalibrationManager::autoCalibration()
{
  ...
  while (EStepperCalibrationStatus::IN_PROGRESS == final_status)
  {
    ...
    ROS_DEBUG("CalibrationManager::autoCalibration - calibration status, %s",
              common::model::StepperCalibrationStatusEnum(final_status).toString().c_str());
  }
  ...
  for (auto const &jState : _joint_states_list)
  {
    if (jState && jState->isStepper())
    {
      ...
      if (_stepper_bus_interface)
      {
        ...
        ROS_INFO("CalibrationManager::autoCalibration - Motor %d, calibration cmd result %d ", motor_id,
                 calibration_result);
      }
    }
  }
      ...
      if (state)
      {
        ...
        for (size_t i = 0; i < motor_id_list.size(); i++)
        {
          ...
          if (motor_id == _joint_states_list.at(0)->getId())
          {
            offset_to_send =
                sensor_offset_steps -
                _joint_states_list.at(0)->to_motor_pos(_joint_states_list.at(0)->getLimitPositionMax()) % steps_per_rev;
            if (offset_to_send < 0)
              offset_to_send += steps_per_rev;

            StepperSingleCmd stepper_cmd(EStepperCommandType::CMD_TYPE_POSITION_OFFSET,
                                         _joint_states_list.at(0)->getId(), { offset_to_send, offset_to_send });
            _stepper_bus_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(stepper_cmd));
          }
          else if (motor_id == _joint_states_list.at(1)->getId())
          {
            offset_to_send =
                sensor_offset_steps -
                _joint_states_list.at(1)->to_motor_pos(_joint_states_list.at(1)->getLimitPositionMax()) % steps_per_rev;
            // offset_to_send %= steps_per_rev;
            if (offset_to_send < 0)
              offset_to_send += steps_per_rev;

            StepperSingleCmd stepper_cmd(EStepperCommandType::CMD_TYPE_POSITION_OFFSET,
                                         _joint_states_list.at(1)->getId(), { offset_to_send, offset_to_send });
            _stepper_bus_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(stepper_cmd));
          }
          else if (motor_id == _joint_states_list.at(2)->getId())
          {
            offset_to_send = sensor_offset_steps -
                             _joint_states_list.at(2)->to_motor_pos(_joint_states_list.at(2)->getLimitPositionMin());

            StepperSingleCmd stepper_cmd(EStepperCommandType::CMD_TYPE_POSITION_OFFSET,
                                         _joint_states_list.at(2)->getId(), { offset_to_send, sensor_offset_steps });
            _stepper_bus_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(stepper_cmd));
          }
        }
  ...
  if (pState && pState->isStepper())
  {
    ...
    if (_can_interface && EBusProtocol::CAN == pState->getBusProtocol())
    {
      _can_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(
          StepperSingleCmd(EStepperCommandType::CMD_TYPE_TORQUE, motor_id, { percentage })));
    }
    else if (_ttl_interface && EBusProtocol::TTL == pState->getBusProtocol())
    {
      _ttl_interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
          StepperTtlSingleCmd(EStepperCommandType::CMD_TYPE_TORQUE, motor_id, { percentage })));
    }
  }
}
...
void CalibrationManager::initVelocityProfiles()
{
  ...
  if ("ned2" == _hardware_version)
  {
    for (auto param : _calibration_params_map)
    {
      ...
      StepperTtlSingleCmd cmd_profile(EStepperCommandType::CMD_TYPE_VELOCITY_PROFILE, param.first,
  ...
  if ("ned2" == _hardware_version)
  {
    for (auto const &jState : _joint_states_list)
    {
      if (jState && jState->isStepper() && EBusProtocol::TTL == jState->getBusProtocol())
      {
        ...
        {
          // CMD_TYPE_VELOCITY_PROFILE cmd
          StepperTtlSingleCmd cmd_profile(EStepperCommandType::CMD_TYPE_VELOCITY_PROFILE, stepperState->getId(),
...
void CalibrationManager::moveRobotBeforeCalibration()
{
  ...
  if (_can_interface && _joint_states_list.at(1)->isStepper() &&
      common::model::EBusProtocol::CAN == _joint_states_list.at(1)->getBusProtocol())
  {
    ...
    _can_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(
        StepperSingleCmd(EStepperCommandType::CMD_TYPE_RELATIVE_MOVE, motor_id, { steps, delay })));
  }
  ...
  if (_joint_states_list.at(2)->isStepper())
  {
    ...
    {
      auto steps = static_cast<uint32_t>(_joint_states_list.at(2)->getPosition() +
                                         10 * _joint_states_list.at(2)->getDirection());

      _ttl_interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
          StepperTtlSingleCmd(EStepperCommandType::CMD_TYPE_POSITION, motor_id, { steps })));
    }
  }
      ...
      if (EBusProtocol::CAN == jState->getBusProtocol())
      {
        ...
        if (!_simulation_mode)
          ...
          _can_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(StepperSingleCmd(
              EStepperCommandType::CMD_TYPE_POSITION, motor_id, { jState->to_motor_pos(jState->getHomePosition()) })));
      }
    ...
    if (jState && jState->isStepper())  // only steppers can be calibrated
    {
      ...
      if (pStepperMotorState && pStepperMotorState->isValid())
      {
        ...
        if (_can_interface && EBusProtocol::CAN == pStepperMotorState->getBusProtocol())
        {
          ...
          _can_interface->addSingleCommandToQueue(std::make_unique<StepperSingleCmd>(StepperSingleCmd(
              EStepperCommandType::CMD_TYPE_CALIBRATION, id, { offset, delay, direction, _calibration_timeout })));
        }
        else if (_ttl_interface && EBusProtocol::TTL == pStepperMotorState->getBusProtocol())
        {
          ...
          _ttl_interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(StepperTtlSingleCmd(
              EStepperCommandType::CMD_TYPE_CALIBRATION_SETUP, id, { ttl_direction, stall_threshold })));

          _ttl_interface->addSingleCommandToQueue(std::make_unique<StepperTtlSingleCmd>(
              StepperTtlSingleCmd(EStepperCommandType::CMD_TYPE_CALIBRATION, id)));
  ...
  if (_ttl_interface)
  {
    ...
    for (auto const &jState : _joint_states_list)
    {
      if (jState && jState->isStepper())
      {
        ...
        stepper_cmd.addMotorParam(state->getHardwareType(), state->getId(),
                                  static_cast<int32_t>(state->getHomingAbsPosition()));
        ROS_DEBUG("CalibrationManager::writeHomingAbsPosition - id: %d, homing: %d", state->getId(),
                  static_cast<int32_t>(state->getHomingAbsPosition()));
      }
    }
                                                      ...
                                                      const std::vector<int> &steps_list)
{
  ...
  {
    ROS_ERROR(
        "CalibrationManager::set_motors_calibration_offsets - Corrupted command"
...
bool CalibrationManager::saveHomingAbsPositionToFile()
{
  ...
  if (!returned_error)
  {
    ...
    {
      ROS_WARN("CalibrationManager::saveHomingAbsPositionToFile - Unable to open file : %s",
               _calibration_file_name.c_str());
    }
  }
  else
    ...
    if (id_homing_map.size() == _calibration_params_map.size())
    {
      for (auto const &jState : _joint_states_list)
      {
        if (jState && jState->isStepper())
        {
          ...
          ROS_DEBUG("CalibrationManager::readHomingAbsPositionFromFile - id: %d, homing_pos: %u", state->getId(),
                    state->getHomingAbsPosition());
        }
      }
      res = true;
    }
