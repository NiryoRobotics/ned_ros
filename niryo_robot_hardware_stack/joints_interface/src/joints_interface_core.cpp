    ...
    void JointsInterfaceCore::rosControlLoop()
    {
        ...
        while (ros::ok())
        {
            if (_enable_control_loop)
            {
                ...
                ROS_DEBUG_COND(isFreqMet, "JointsInterfaceCore::rosControlLoop : freq met : expected (%f s) vs actual (%f s)", control_loop_rate.expectedCycleTime().toSec(), control_loop_rate.cycleTime().toSec());
