 ...
 * @param can_interface
 */
JointsInterfaceCore::JointsInterfaceCore(ros::NodeHandle &rootnh, ros::NodeHandle &robot_hwnh,
...
bool JointsInterfaceCore::init(ros::NodeHandle &nh)
{
  ...
  _previous_state_learning_mode =
      (("ned2" != _hardware_version && "ned3pro" != _hardware_version) && !_simulation_mode);
...
void JointsInterfaceCore::startServices(ros::NodeHandle &nh)
{
  ...
  _factory_calibrate_motors_server = nh.advertiseService("/niryo_robot/joints_interface/factory_calibrate_motors",
                                                         &JointsInterfaceCore::_callbackFactoryCalibrateMotors, this);
}
...
{
  _trajectory_result_subscriber = nh.subscribe(_joint_controller_name + "/follow_joint_trajectory/result", 10,
                                               &JointsInterfaceCore::_callbackTrajectoryResult, this);
}
...
bool JointsInterfaceCore::rebootAll(bool torque_on)
{
  ...
  activateLearningMode((("ned2" != _hardware_version && "ned3pro" != _hardware_version) && !_simulation_mode), result,
                       result_message);
  _previous_state_learning_mode =
      (("ned2" != _hardware_version && "ned3pro" != _hardware_version) && !_simulation_mode);
...
void JointsInterfaceCore::rosControlLoop()
{
  ...
  while (ros::ok())
  {
    if (_enable_control_loop && !_estop_flag)
    {
      ...
      // ROS_DEBUG_COND(!isFreqMet, "JointsInterfaceCore::rosControlLoop : freq not met : expected (%f s) vs actual (%f
      // s)", _control_loop_rate.expectedCycleTime().toSec(),
 ...
 *
 * NED2
 * Callback call when we have to make a discontinuity of position command in ros control (like if we have a collision,
 * we have to update the position command to be equal with the position actual to avoid joints continue moving) This way
 * is a treat to block the write function of robot and wait a little bit and then reset controller to make ros
 * controller update cmd. If ros controller is reset rapidly, The pid return unvalid position.
 */
 ...
 * @return
 */
bool JointsInterfaceCore::_callbackFactoryCalibrateMotors(FactoryCalibration::Request &req,
                                                          FactoryCalibration::Response &res)
{
