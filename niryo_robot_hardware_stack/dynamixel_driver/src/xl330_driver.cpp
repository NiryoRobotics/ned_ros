...
{
    return _dxlPacketHandler->write2ByteTxOnly(_dxlPortHandler.get(), id, XL330_ADDR_GOAL_SPEED, (uint16_t)velocity);
}

int XL330Driver::setGoalTorque(uint8_t id, uint32_t torque)
{
    return _dxlPacketHandler->write2ByteTxOnly(_dxlPortHandler.get(), id, XL330_ADDR_GOAL_TORQUE, (uint16_t)torque);
}
...
{
    return _dxlPacketHandler->write1ByteTxOnly(_dxlPortHandler.get(), id, XL330_ADDR_LIMIT_TEMPERATURE, (uint8_t)temperature);
}

int XL330Driver::setMaxTorque(uint8_t id, uint32_t torque)
{
    return _dxlPacketHandler->write1ByteTxOnly(_dxlPortHandler.get(), id, XL330_ADDR_MAX_TORQUE, (uint8_t)torque);
}

int XL330Driver::setReturnLevel(uint8_t id, uint32_t return_level)
{
    return _dxlPacketHandler->write1ByteTxOnly(_dxlPortHandler.get(), id, XL330_ADDR_RETURN_LEVEL, (uint8_t)return_level);
}
...
{
    return syncWrite2Bytes(XL330_ADDR_GOAL_SPEED, id_list, velocity_list);
}
int XL330Driver::syncWriteTorqueGoal(std::vector<uint8_t> &id_list, std::vector<uint32_t> &torque_list)
{
    return syncWrite2Bytes(XL330_ADDR_GOAL_TORQUE, id_list, torque_list);
}
...
{
    return read2Bytes(XL330_ADDR_PRESENT_SPEED, id, present_velocity);
}

int XL330Driver::readLoad(uint8_t id, uint32_t *present_load)
{
    return read2Bytes(XL330_ADDR_PRESENT_LOAD, id, present_load);
}
...
{
    return read1Byte(XL330_ADDR_LIMIT_TEMPERATURE, id, limit_temperature);
}

int XL330Driver::readMaxTorque(uint8_t id, uint32_t *max_torque)
{
    return read2Bytes(XL330_ADDR_MAX_TORQUE, id, max_torque);
}

int XL330Driver::readReturnLevel(uint8_t id, uint32_t *return_level)
{
    return read1Byte(XL330_ADDR_RETURN_LEVEL, id, return_level);
}
...
{
    return syncRead(XL330_ADDR_PRESENT_SPEED, DXL_LEN_TWO_BYTES, id_list, velocity_list);
}
int XL330Driver::syncReadLoad(std::vector<uint8_t> &id_list, std::vector<uint32_t> &load_list)
{
    return syncRead(XL330_ADDR_PRESENT_LOAD, DXL_LEN_TWO_BYTES, id_list, load_list);
}
