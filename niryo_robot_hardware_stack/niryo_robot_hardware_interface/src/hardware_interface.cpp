...
void HardwareInterface::initNodes(ros::NodeHandle &nh)
{
  ...
  if (_ttl_enabled)
  {
    ...
    if (_end_effector_enabled)
    {
      ...
      _end_effector_interface =
          std::make_shared<end_effector_interface::EndEffectorInterfaceCore>(nh_ee, _ttl_interface);
    }
    ros::Duration(0.25).sleep();
  }
  else
  ...
  _joints_interface =
      std::make_shared<joints_interface::JointsInterfaceCore>(nh, nh_joints, _ttl_interface, _can_interface);
  ros::Duration(0.25).sleep();

  ROS_DEBUG("HardwareInterface::initNodes - Start Conveyor Interface Node");
  ros::NodeHandle nh_conveyor(nh, "conveyor");
  _conveyor_interface =
      std::make_shared<conveyor_interface::ConveyorInterfaceCore>(nh_conveyor, _ttl_interface, _can_interface);
  ros::Duration(0.25).sleep();
}
...
{
  _hw_status_publisher =
      nh.advertise<niryo_robot_msgs::HardwareStatus>("/niryo_robot_hardware_interface/hardware_status", 10);

  _hw_status_publisher_timer =
      nh.createTimer(_hw_status_publisher_duration, &HardwareInterface::_publishHardwareStatus, this);

  _sw_version_publisher =
      nh.advertise<niryo_robot_msgs::SoftwareVersion>("/niryo_robot_hardware_interface/software_version", 10);

  _sw_version_publisher_timer =
      nh.createTimer(_sw_version_publisher_duration, &HardwareInterface::_publishSoftwareVersion, this);
...
{
  _robot_status_subscriber =
      nh.subscribe("/niryo_robot_status/robot_status", 10, &HardwareInterface::_callbackRobotStatus, this);
}
