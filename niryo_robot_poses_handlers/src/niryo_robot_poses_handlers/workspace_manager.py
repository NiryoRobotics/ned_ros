...
class Workspace(object):
    ...
    def to_dict(self):
        ...
        dict_["robot_poses"] = self.robot_poses
    ...
    def from_dict(cls, dict_):
        ws = cls(dict_["name"], dict_["description"])
        ws.robot_poses = dict_["robot_poses"]
    ...
    def get_value(self):
        return self.points, self.robot_poses
...
class WorkspaceManager(FileManager):
    ...
    def create(self, workspace_name, points, robot_poses=None, description=""):
        ...
        if self.exists(workspace_name):
            raise NiryoRobotFileException("Workspace {} already exists".format(workspace_name))
        ...
        workspace = Workspace(workspace_name, description)
        workspace.points = points
        workspace.robot_poses = robot_poses if robot_poses is not None else []
