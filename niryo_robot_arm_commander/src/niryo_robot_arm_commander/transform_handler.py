...
class ArmTCPTransformHandler:
    ...
    def tcp_to_ee_link_pose_target(self, pose, ee_link):
        try:
            ...
            transform_world_to_tcp_target = self.transform_from_pose(pose, "base_link", "tcp_target", stamp=stamp)
            self.__tf_buffer.set_transform(transform_world_to_tcp_target, "default_authority")
            ...
            transform_tcp_to_ee_link.header.stamp = stamp
            self.__tf_buffer.set_transform(transform_tcp_to_ee_link, "default_authority")

            ee_link_target_transform = self.__tf_buffer.lookup_transform("base_link", "ee_link_target", rospy.Time(0))
