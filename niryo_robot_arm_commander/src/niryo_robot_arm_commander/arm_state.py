...
class ArmState(object):
    ...
    def __init__(self):
        ...
        rospy.logdebug("ArmCommander.init - joint_controller_name: %s",
                       self.__joints_name)

        # Getting reference frame from robot_commander.launch
        self.__reference_frame = rospy.get_param("~reference_frame")
        rospy.logdebug("ArmCommander.init - reference_frame: %s",
                       self.__reference_frame)
        ...
        self.__max_velocity_scaling_percentage = int(
            self.__velocity_scaling_factor *
            self.__velocity_percentage_scaling_factor)

        self.__max_acceleration_scaling_percentage = int(
        ...
        rospy.Service(
            ...
            SetInt, self.__callback_set_max_velocity_scaling_factor)

        rospy.Service('/niryo_robot_arm_commander/set_acceleration_factor',
                      SetInt, self.__callback_set_acceleration_factor)

        self.__write_custom_value_service = rospy.ServiceProxy(
            "/niryo_robot/ttl_driver/send_custom_value", WriteCustomValue)
        ...
        self.check_state_validity = rospy.ServiceProxy('check_state_validity',
                                                       GetStateValidity)

        self.__learning_mode_on = None
        rospy.Subscriber('/niryo_robot/learning_mode/state', Bool,
                         self.__callback_sub_learning_mode)
        self.__learning_mode_service = rospy.ServiceProxy(
            "/niryo_robot/learning_mode/activate", SetBool)

        self.__hardware_status = rospy.wait_for_message(
        ...
        self.__arm_param_validator = ArmParametersValidator(
            rospy.get_param("/niryo_robot/robot_command_validation"))
    ...
    def __callback_set_max_velocity_scaling_factor(self, req):
        rospy.logdebug(
            "ArmCommander - __callback_set_max_velocity_scaling_factor: %d",
            req.value)
        ...
        try:
            ...
            self.__set_max_acceleration_scaling_percentage(
            ...
            while rospy.has_param(stepper_param_ns + str(i) +
                                  '/v_max') and rospy.get_param(
                                      stepper_param_ns + str(i) + '/a_max'):
                default_vmax = rospy.get_param(stepper_param_ns + str(i) +
                                               '/v_max')
                default_amax = rospy.get_param(stepper_param_ns + str(i) +
                                               '/a_max')

                _id = i + STEPPER_ID_OFFSET
                scaled_velocity = int(
                    default_vmax * self.__velocity_scaling_factor *
                    RADIAN_PER_SECONDS_TO_RPM * 1000)  # rpm to stepper value
                self.__write_custom_value_service(
                    id=_id,
                    value=scaled_velocity,
                    reg_address=VEL_PROFILE_ADDR,
                    byte_number=VEL_PROFILE_BYTE_LEN)
                scaled_acceleration = int(default_amax *
                                          self.__acceleration_scaling_factor *
                                          RADIAN_PER_SECONDS_SQ_TO_RPM_SQ /
                                          214.577)  # rpm2 to stepper value
                self.__write_custom_value_service(
                    id=_id,
            ...
            while rospy.has_param(dxl_param_ns + str(i) +
                                  '/velocity_profile') and rospy.get_param(
                                      dxl_param_ns + str(i) +
                                      '/acceleration_profile'):
                default_vmax = rospy.get_param(dxl_param_ns + str(i) +
                                               '/velocity_profile')
                default_amax = rospy.get_param(dxl_param_ns + str(i) +
                                               '/acceleration_profile')

                _id = i + DXL_ID_OFFSET
                scaled_velocity = int(default_vmax *
                                      self.__velocity_scaling_factor)
                self.__write_custom_value_service(
                    id=_id,
                    value=scaled_velocity,
            ...
            return {
                'status': CommandStatus.ARM_COMMANDER_FAILURE,
                'message': e.message
            }
    ...
    def __callback_set_acceleration_factor(self, req):
        rospy.logdebug("ArmCommander - __callback_set_acceleration_factor: %d",
                       req.value)
    ...
    def __publish_velocity_scaling_percentage(self):
        ...
        :return: None
        """
        rospy.logdebug("ArmCommander.init - Velocity_scaling_percentage: %d",
    ...
    def __set_max_velocity_scaling_percentage(self, percentage):
        ...
        self.__arm.set_max_velocity_scaling_factor(
            self.__velocity_scaling_factor)
        self.__publish_velocity_scaling_percentage()
    ...
    def __set_max_acceleration_scaling_percentage(self, percentage):
        ...
        self.__arm.set_max_acceleration_scaling_factor(
            self.__acceleration_scaling_factor)
        self.__publish_acceleration_scaling_percentage()
    ...
    def set_learning_mode(self, set_bool):
        ...
        try:
            return self.__learning_mode_service(
                set_bool).status == CommandStatus.SUCCESS
    ...
    def __init_move_group_commander(self):
        ...
        move_group_commander_name = rospy.get_param(
            "~move_group_commander_name")
        rospy.logdebug("ArmCommander.init - move_group_commander_name: %s",
                       move_group_commander_name)

        self.__arm = moveit_commander.MoveGroupCommander(
            move_group_commander_name)
        ...
        self.__arm.set_goal_joint_tolerance(
            rospy.get_param("~goal_joint_tolerance"))
        self.__arm.set_goal_position_tolerance(
            rospy.get_param("~goal_position_tolerance"))
        self.__arm.set_goal_orientation_tolerance(
            rospy.get_param("~goal_orientation_tolerance"))

        rospy.loginfo(
            "Arm commander - MoveIt! successfully connected to move_group '{}'"
            .format(self.__arm.get_name()))
        rospy.logdebug(
            "Arm commander - MoveIt! will move '{}' in the planning_frame '{}'"
            .format(self.__arm.get_end_effector_link(),
                    self.__arm.get_planning_frame()))
