...
class DailyReportHandler:
    ...
    def __init__(self, cloud_api, reports_path):
        ...
        add_report_response = system_api_client.add_file_path('daily_report', report_name, report_path)
        if add_report_response.success:
            self.__current_id = add_report_response.data['id']
        else:
            rospy.logerr('Unable to add the daily report to the database: {}'.format(add_report_response.detail))
            self.__current_id = uuid4()
    ...
    def __new_day_callback(self, _req):
        ...
        try:
            ...
            system_api_client.rm_file_path(_id=self.__current_id)
    ...
    def __send_failed_daily_reports(self):
        daily_reports_response = system_api_client.get_file_paths('daily_report')
        if daily_reports_response.success:
            for report in daily_reports_response.data:
