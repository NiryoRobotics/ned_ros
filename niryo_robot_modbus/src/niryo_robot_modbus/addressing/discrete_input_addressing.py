...
def get_addressing(ros_wrapper: NiryoRosWrapper):
    ...
    :returns: A dictionary mapping integer indices and keys to WrapperAddress objects.
    ...
    return {
        ...
        **WrapperAddress.dynamic_addressing(0,
                                            n_digital_inputs,
                                            lambda ix: ros_wrapper.digital_read(dios[ix]) == PinState.HIGH),
        100:  # freedrive / learning mode
        WrapperAddress(read=lambda: ros_wrapper.get_learning_mode()),
        110:  # calibration needed
        WrapperAddress(read=lambda: ros_wrapper.get_hardware_status().calibration_needed),
        111:  # calibration in progress
        WrapperAddress(read=lambda: ros_wrapper.get_hardware_status().calibration_in_progress),
        112:  # motor connection ok
        WrapperAddress(read=lambda: ros_wrapper.get_hardware_status().connection_up),
        # conveyor bus ok
        **WrapperAddress.dynamic_addressing(200,
                                            n_conveyors,
                                            lambda ix: ros_wrapper.get_conveyors_feedback()[ix].connection_state),
        # conveyor is running
        **WrapperAddress.dynamic_addressing(210,
                                            n_conveyors,
                                            lambda ix: ros_wrapper.get_conveyors_feedback()[ix].running),
        # conveyor direction
        **WrapperAddress.dynamic_addressing(220,
                                            n_conveyors,
